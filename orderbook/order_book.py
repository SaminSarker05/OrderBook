...
class OrderBook:
    ...
    - Bids: sorted in decreasing order (highest buyer first)
    - Asks: sorted in ascending order (lowest seller first)
    """
    def __init__(self):
        # map each price to a PriceLevel object in sorted dict
        # TODO(samin): make own sorteddict and red/black tree
    ...
    def add_order(self, order: Order):
        """Add a resting order to order book in logP time."""
    ...
    def cancel_order(self, order_id: int) -> bool:
        ...
        if price_level:
            # remove order from underlying doubly linked list
    ...
    def get_l2_snapshot(self, depth: int = 20) -> tuple[
                                                    list[
        ...
        """
        Display level 2 data of orderbook - best n prices and total quantity 
        for bid / ask sides of orderbook. Returns price and volume pairs up 
        to specified depth.
        """
        bids_l2, asks_l2 = [], []
        for price, level in list(self.bids.items())[:depth]:
            bids_l2.append((price, level.total_quantity))
        
        for price, level in list(self.asks.items())[:depth]:
            asks_l2.append((price, level.total_quantity))
